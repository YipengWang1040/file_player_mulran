  ...
  while(1){
    ...
    while(!radarpolar_thread_.data_queue_.empty())
    {
      ...
      else
      {
        ...
        radarpolar_image = imread(current_radarpolar_name, cv::ImreadModes::IMREAD_GRAYSCALE);
      ...
      if(current_img_index < radarpolar_file_list_.size()-2)
      {
        ...
        cv::Mat radarpolar_image;
        radarpolar_image = imread(next_radarpolar_name, cv::ImreadModes::IMREAD_COLOR);
